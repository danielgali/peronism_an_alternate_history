...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['pl', 'pc', 'ucr', 'pdp', 'pdn', 'ps', 'nsdap', 'other'];
    }
    if (!partyColors) {
        partyColors = {'pl': '#0047AB', 'pc': #FF0000', 'ucr': '#E10019', 'pdp': '#005C9E', 'pdn': '#005C9E', 'ps': '#FF9900', 'nsdap': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'pl': 'PL', 'pc': 'PC', 'ucr': 'UCR', 'pdp': 'PDP', 'pdn': 'PDN', 'dnvp': 'PS', 'nsdap': 'NSDAP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          const maxPL = d3.max(data, d => d.pl);
          const maxUCR = d3.max(data, d => d.nsdap);
          dataMax = maxPL >= maxUCR ? maxPL + 10 : maxUCR + 10;
          dataMin = 0;
      }
